...
enum torf_t { false, true };

FILE *FileOpen(byte *szFileName, byte *szMode)
{
  FILE *fpFp;
...
void SplitData(byte *szBuf, byte *szDo, byte *szGun, byte *szMyun, byte *szRi, byte *szTail)
{
  sscanf(szBuf, "%s %s %s %s %s", szDo, szGun, szMyun, szRi, szTail);
}

void BufferClear(byte *szDo, byte *szGun, byte *szMyun, byte *szRi, byte *szTail)
{
  szDo[0] = szGun[0] = szMyun[0] = szRi[0] = szTail[0] = NULL;
}

int GetMyunNumber(byte *szMyun)
{
  int nCnt, nNum;
...
byte *OmitMyunNumber(byte *szMyun)
{
  int nCnt;
...
byte *OmitMaxNumber(byte *szMyun)
{
  int nCnt;
...
void main(void)
{
  byte szBuf[LINEBUFSIZE], *pZip, *pAddr;
